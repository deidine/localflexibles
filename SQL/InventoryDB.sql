...
--
-- Host: 127.0.0.1
-- Generation Time: Oct 09, 2023 at 10:38 PM
...
END$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `LoanAmount` (IN `dateSalle` DATE, OUT `SaleAmount` INT)   BEGIN SELECT COUNT(*) INTO SaleAmount FROM loansinfo WHERE DATE(date) = dateSalle;  
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `TotalLoan` (IN `date` DATE, OUT `totalLoan` DECIMAL(10,2))   BEGIN SELECT SUM(total_paye) INTO totalLoan FROM loansinfo WHERE DATE(date) = date;  
END$$
...
('cl1', 'deidine', 'atar', '89989'),
('inconue', 'inconue', 'inconue', '123456789');

-- --------------------------------------------------------

--
-- Stand-in structure for view `loandata`
...
--
-- Table structure for table `loansinfo`
--
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `loandata`  AS SELECT `loin`.`loansid` AS `loansid`, `loin`.`date` AS `date`, `loin`.`customercode` AS `customercode`, `loin`.`total_paye` AS `total_paye`, `loin`.`recu` AS `recu`, `loin`.`changeMony` AS `changeMony`, `loin`.`soldby` AS `soldby`, `lodt`.`productcode` AS `productcode`, `lodt`.`quantity` AS `quantity` FROM (((`loansinfo` `loin` join `loan_detail` `lodt` on(`loin`.`loansid` = `lodt`.`loansid`)) join `products` `p` on(`lodt`.`productcode` = `p`.`productcode`)) join `users` on(`loin`.`soldby` = `users`.`name`))  ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `salledata`  AS SELECT `sin`.`salesid` AS `salesid`, `sin`.`date` AS `date`, `sin`.`customercode` AS `customercode`, `sin`.`total_paye` AS `total_paye`, `sin`.`recu` AS `recu`, `sin`.`changeMony` AS `changeMony`, `sin`.`soldby` AS `soldby`, `sldt`.`productcode` AS `productcode`, `sldt`.`quantity` AS `quantity` FROM (((`salesinfo` `sin` join `sale_detail` `sldt` on(`sin`.`salesid` = `sldt`.`salesid`)) join `products` `p` on(`sldt`.`productcode` = `p`.`productcode`)) join `users` on(`sin`.`soldby` = `users`.`name`))  ;
...
SQL SECURITY DEFINER VIEW `loanraport`  AS SELECT 
`loninf`.`date` AS `date`,cust.fullname ,`loninf`.`total_paye` AS `total_paye`,
 `loninf`.`loansid` AS `loansid`, `loninf`.`soldby` AS `soldby` 
 FROM (`loansinfo` `loninf` join `users` on(`loninf`.`soldby` = `users`.`username`) 
 join customers as cust on cust.customercode=loninf.customercode)  ;
...
--
-- Indexes for table `loansinfo`
--
ALTER TABLE `loansinfo`
...
--
-- AUTO_INCREMENT for table `loansinfo`
--
ALTER TABLE `loansinfo`
  MODIFY `loansid` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=3;
...
--
-- Constraints for table `loansinfo`
--
ALTER TABLE `loansinfo`
  ADD CONSTRAINT `loansinfo_ibfk_2` FOREIGN KEY (`customercode`) REFERENCES `customers` (`customercode`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- Constraints for table `loan_detail`
--
ALTER TABLE `loan_detail`
  ADD CONSTRAINT `loan_detail_ibfk_1` FOREIGN KEY (`productcode`) REFERENCES `products` (`productcode`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `loan_detail_ibfk_2` FOREIGN KEY (`loansid`) REFERENCES `loansinfo` (`loansid`) ON DELETE CASCADE ON UPDATE CASCADE;
